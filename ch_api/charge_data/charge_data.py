    ...
    if does_finished_list_exist("charge"):
        if prompt_user_to_continue("Would you like to narrow down the unique company list with these additional lists"):
            try:
                ...
                company_list["uk_companies"] = trim_company_list_file(company_list["uk_companies"], "charge")
            except Exception as e:
                print(e)
...
async def failed_list_scrape():
    ...
    company_list["uk_companies"] = read_failed_company_list_file("charge")
...
async def scrape_charge_data():
    
    create_lock("fetch_charge_data")
